...
class User(Base):
    ...
    auto_login_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    total_payment: Mapped[float] = mapped_column(Numeric(12, 2), nullable=False, default=0)
    primary_device: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
    admin_memo: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    widget_config: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON
    # 식단 분석용 신체정보
...
class ExpenseCategory(Base):
    ...
    icon: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)       # 이모지 또는 아이콘 코드
    code: Mapped[Optional[str]] = mapped_column(String(30), nullable=True, index=True)  # 'REGULAR','SALARY' 등
...
class Expense(Base):
    ...
    category: Mapped[Optional[str]] = mapped_column(String(100))       # 레거시 텍스트 카테고리
    ...
    converted_amount: Mapped[Optional[float]] = mapped_column(Numeric(14, 2), nullable=True)  # USD 환산액
    exchange_rate: Mapped[Optional[float]] = mapped_column(Numeric(14, 6), nullable=True)     # 적용 환율
...
class ExpenseBudget(Base):
    ...
    month: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)         # NULL = 연간, 1~12 = 월별
...
class DietAnalysis(Base):
    ...
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    date: Mapped[date] = mapped_column(Date, nullable=False, index=True)
    nutrition_analysis: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    recommendations: Mapped[Optional[str]] = mapped_column(Text, nullable=True)   # JSON 배열 문자열
    warnings: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    raw_meals: Mapped[Optional[str]] = mapped_column(Text, nullable=True)         # 분석 당시 식단 스냅샷 JSON
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), onupdate=func.now())
...
class Todo(Base):
    """수동 할 일 체크리스트. start_date~due_date 범위 내 날짜에 표시."""
    __tablename__ = "todos"
    ...
    start_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True)  # null = 생성일 기준
    due_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True)    # null = 영구 표시
...
class Stock(Base):
    ...
    avg_price: Mapped[Optional[float]] = mapped_column(Numeric(14, 4))
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), onupdate=func.now())
...
class StockPriceHistory(Base):
    ...
    current_price: Mapped[Optional[float]] = mapped_column(Numeric(14, 4))
    prev_close: Mapped[Optional[float]] = mapped_column(Numeric(14, 4))
    change_amount: Mapped[Optional[float]] = mapped_column(Numeric(14, 4))
    change_percent: Mapped[Optional[float]] = mapped_column(Numeric(8, 4))
...
class YoutubeChannel(Base):
    ...
    channel_url: Mapped[Optional[str]] = mapped_column(String(2000))
    category: Mapped[Optional[str]] = mapped_column(String(100))
    description: Mapped[Optional[str]] = mapped_column(String(500))
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
...
class DailyPortfolioSnapshot(Base):
    ...
    snapshot_date: Mapped[date] = mapped_column(Date, nullable=False, index=True)
    usd_krw: Mapped[Optional[float]] = mapped_column(Float)          # 환율
    total_usd: Mapped[Optional[float]] = mapped_column(Float)        # USD 그룹 합계
    total_krw: Mapped[Optional[float]] = mapped_column(Float)        # KRW 그룹 합계
    total_krw_equiv: Mapped[Optional[float]] = mapped_column(Float)  # 원화환산 전체 합계
    data: Mapped[Optional[str]] = mapped_column(Text)                # JSON — 그룹/종목 상세
    realized_pl: Mapped[Optional[float]] = mapped_column(Float, nullable=True)  # 실현 손익 합계
    saved_by: Mapped[str] = mapped_column(String(20), default="frontend")  # 'frontend' | 'scheduler'
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), onupdate=func.now())


class GoogleServiceToken(Base):
    """Google OAuth 서비스 토큰 테이블.

    calendar, youtube 등 여러 Google 서비스 연동을 하나의 테이블에서 관리.
    (user_id, service_type) 쌍이 유니크 — 서비스별로 별도 계정 연동 가능.
    """
    __tablename__ = "google_service_tokens"
    __table_args__ = (
    ...
    service_type: Mapped[str] = mapped_column(String(30), nullable=False)  # 'calendar' | 'youtube'
