...
---
## 2026-06-09 — USD/KRW 판단 기준: 그룹명(_CAT_META) → currency 필드로 전환
**결정:** 포트폴리오 그룹의 USD/KRW 판단 기준을 그룹명(_CAT_META 역방향 맵) 대신 `currency` 필드로 전환. 그룹명이 알려진 _CAT_META 값("Robinhood", "KOR Stock" 등)이면 기존 매핑 우선 사용 (하위호환), 그 외는 `currency` 필드로 직접 판단.
**이유:** 사용자가 그룹명을 "새 그룹", "내 포트폴리오" 등 임의로 변경할 경우 _CAT_META 역방향 맵 조회 실패 → 카테고리 오판 → 환율 환산 오류. `currency` 필드는 AdminPage select에서 명시적으로 설정하므로 항상 신뢰 가능.
**대안:** 그룹명만으로 판단 (기존 방식) — 커스텀 그룹명에서 환산 오류 발생. currency만으로 판단 — 기존 레거시 그룹("Robinhood" 등) 일부 엣지케이스 리스크.
**파일:** `routers/portfolio.py`, `frontend/src/pages/index/StockStatsOverlay.jsx`

---
## 2026-06-09 — 같은 그룹 내 중복 종목 추가 방지 (포트폴리오 데이터 무결성)
**결정:** AdminPage.jsx의 `addStock()` 함수에서 같은 포트폴리오 그룹 내에 이미 존재하는 티커(대소문자 무관)를 추가하려 할 때 에러 토스트를 표시하고 추가를 중단하는 유효성 검사 로직 추가.
**이유:** 같은 포트폴리오 그룹 내에 동일 종목을 중복으로 보유하는 것은 포트폴리오 데이터 무결성 문제. 사용자가 실수로 같은 종목을 두 번 추가하는 경우를 사전에 방지하여 나중에 수정하는 비용 절감. 반면 서로 다른 그룹 간에는 같은 티커를 허용하여 (예: KR 포트폴리오와 US 포트폴리오에서 글로벌 종목 추적) 유연성 유지.
**대안:**
- 중복 허용: 사용자가 데이터를 정리해야 하는 번거로움, 포트폴리오 분석 오류 가능성
- 전역 중복 방지 (모든 그룹): 카테고리/통화별 포트폴리오 분리 전략 제약
- 선택한 방식: 그룹 범위 내 유효성 검사로 그룹별 독립성 유지하면서 중복 방지
...
---
## 2026-06-09 16:25 — 누적 투자 차트에 통화 환율 반영 (다중 통화 포트폴리오 정규화)
**결정:** StockStatsOverlay.jsx의 `computeStockStats()` 함수에서 매수 거래의 금액 계산 시 통화 환율을 적용하는 로직 추가. USD 그룹의 매수는 `rawAmt * fxRate`로 계산하고, KRW 그룹은 원본 금액 유지.
**이유:** 다중 통화 포트폴리오(USD, KRW)의 누적 투자 차트를 올바르게 표시하기 위해 각 매수의 금액을 통화에 맞게 정규화 필요. USD 거래는 KRW 환율을 곱하여 KRW 기준으로 변환하고, KRW는 그대로 유지하면 모든 매수가 동일 통화(KRW) 기준으로 누적되어 차트의 y축이 일관성 있게 표시됨.
**대안:**
- 환율 미적용: USD와 KRW 금액이 섞여 차트의 y축이 혼동스러움
- 프론트 렌더링 단계에서 변환: 매번 렌더링 시 계산 필요, 성능 저하
- 선택한 방식: 데이터 집계 단계에서 미리 정규화하여 저장, 렌더링 시 바로 사용 가능
