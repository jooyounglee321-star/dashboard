...
---
## 2026-09-15 (2차) — 기존 매입 기록의 배당 재투자 소급 재분류: 건별 수동 태그 대신 AI 재캡처 매칭 방식 채택

**결정:** 이미 "매입"으로 저장된 과거 거래 중 실제로는 배당 재투자였던 건을 사용자가
가진 실제 명세서 스크린샷을 다시 업로드하는 것만으로 일괄 찾아내 재분류하도록 구현.
AI가 `type: "reinvest"`로 인식했는데 (ticker, date, qty, price)가 기존 매입 기록과
정확히 일치하면 "재분류 후보"로 제안하고, 사용자가 선택해 저장하면 해당 기록에
`source: "reinvestment"` 태그 + 배당금 내역 1건이 자동 추가됨.

**이유:** 실사용자 계좌를 확인해보니 재투자 대상 후보가 NVDY만이 아니라 여러 종목에
걸쳐 40건 넘게 존재. 거래내역 수정화면에서 건별로 체크박스를 하나씩 켜는 방식은
이 정도 규모에서는 비현실적. 반면 AI 캡처는 이미 구축된 인프라라 "재분류 후보 탐지"
로직만 추가하면 사용자가 별도 학습 없이 기존 워크플로(스크린샷 업로드)를 그대로
재사용할 수 있음.

**부수 발견 및 수정:** 같은 명세서에 머니마켓/결제계좌(VMFXX) Reinvestment 행도
섞여 있었는데, 이건 실제 주식 수량 변화가 없는(Quantity "—") 현금성 거래라 그대로
캡처하면 수량 0짜리 가짜 "종목"이 등록될 위험이 있었음. 프롬프트에 명시적 제외 규칙을
추가하고, 백엔드에도 수량 0 이하 행을 방어적으로 스킵하는 가드를 추가.

**대안:** 거래내역 수정화면에 체크박스를 추가해 건별 수동 태그(처음 제안했던 방식) —
소량 데이터엔 적합하지만 이번처럼 수십 건 규모에선 사용성이 떨어져 채택하지 않음.
다만 향후 소량의 개별 정정이 필요한 경우를 위해 이 방식도 별도로 고려 가능.

**영향 범위:** `routers/portfolio.py`(`PROMPT` 수정, `parse_transactions_from_images`의
`retag_candidates` 응답 필드, 신규 `/api/portfolio/retag-reinvestment` 엔드포인트),
`frontend/src/pages/index/StockSettingsModal.jsx`(`CaptureUploadPanel`의 재분류 후보
섹션). DB 스키마 변경 없음.

**주의:** 매칭은 ticker+date+qty+price가 정확히 일치하는 매입 1건에 대해서만 이뤄짐.
스크린샷의 수량/단가가 소수점 반올림 등으로 기존 저장값과 미세하게 다르면 매칭
안 될 수 있음 — 이 경우 후보 목록에 나타나지 않으므로 수동 확인이 필요함.
...
"FIFO(가장 먼저 산 주식부터 매도된 것으로 간주)"로 변경.

**이유:** 실제 증권사 계좌(Vanguard)와 대조해보니 매매가 잦은 종목에서 평균단가가 크게
어긋남을 발견. Vanguard 계좌가 명시적으로 "FIFO"를 cost-basis 방식으로 쓰고 있었고,
미국 증권사 대부분이 주식/ETF에 대해 FIFO를 기본값으로 사용함. 대시보드를 FIFO로
맞추면 사용자가 실제 계좌와 비교했을 때 일치하는 숫자를 보게 됨.

**영향 범위:** `frontend/src/utils/calcStock.js`(신규 `fifoCalc`), `StockSettingsModal.jsx`,
`StockStatsOverlay.jsx`, `routers/_shared.py`(신규 `fifo_calc`), `routers/portfolio.py`의
`/realized-pl`·`/period-pl`·히스토리 백필. 프론트·백엔드 양쪽에 동일 로직을 각 언어로
구현했고, 실제 사용자 거래내역으로 두 구현 결과가 일치함을 확인.

**대안:**
- 기존 방식(단순 가중평균): 계산은 간단하지만 매매가 잦을수록 실제 계좌와 괴리가 커짐
- Specific Lot(건별 지정): 세금 최적화엔 유리하지만 사용자가 매도 시마다 어떤 매수건을
  소진할지 직접 골라야 해서 이 앱의 "빠르게 기록하는" 용도에 안 맞음
- 선택한 방식(FIFO): 대부분 증권사의 기본값과 동일해 실제 계좌와 자연스럽게 맞아떨어지고,
  사용자가 별도로 신경 쓸 것 없이 매수/매도만 날짜순으로 입력하면 됨

**주의:** 매수/매도에 날짜가 없는 레거시 데이터는 항상 "가장 이른 시점"으로 취급해 FIFO
큐 맨 앞에 정렬됨 (기존 로직의 "날짜 없으면 항상 포함" 하위호환 방침과 동일한 원칙).

---
## 2026-09-13 — 탈퇴 자동삭제 job: 전체 배치 트랜잭션 → 유저별 개별 트랜잭션 + 테이블별 SAVEPOINT

**결정:** `_delete_pending_withdrawals_job()`을 "대상자 전체를 한 트랜잭션으로 처리 후 한 번에 commit"
방식에서 "유저 1명당 독립 트랜잭션으로 즉시 commit/rollback" 방식으로 변경. 테이블별 삭제도
SAVEPOINT로 감싸 개별 실패를 격리.

**이유:** 기존 방식은 대상자 중 한 명이라도 삭제에 실패하면(FK 제약, 존재하지 않는 테이블 등)
예외가 전체를 롤백시켜 그 배치의 다른 정상 대상자까지 전부 처리되지 않는 구조였음. 실제로
`datetime`/`timezone` 이름 충돌 버그와 맞물려 이 job이 배포 이후 단 한 번도 정상 실행되지
못했던 것을 발견하면서, 앞으로 비슷한 개별 실패가 있어도 전체가 멈추지 않도록 격리 필요성 확인.

**대안:**
- 기존 방식(전체 배치 1트랜잭션): 구현은 단순하지만 한 명의 실패가 전체를 막음 — 실제 운영 장애 원인이 됨
- 선택한 방식(유저별 개별 트랜잭션 + 테이블별 SAVEPOINT): 코드는 약간 길어지지만 부분 실패에
  강하고, 실패한 유저만 다음날 재시도되며 나머지는 정상 처리됨

**파일:** `main.py` (`_delete_pending_withdrawals_job`)

---
## 2026-09-13 — 포트폴리오 CASH(납입금) 추적 기능 완전 폐지, 순수 주식거래 기반 계산으로 전환

**결정:** 그룹별 현금 납입금(contributions) 입력·잔고 추적·수익률 계산 반영 기능을 전면 삭제.
포트폴리오 수익률은 오로지 매수/매도(주식거래) 데이터만으로 계산.

**이유:** 사용자가 현금 입출금까지 별도로 기록·관리하는 방식이 실사용 흐름과 맞지 않고
번거로움. 매수총액(원금) 대비 평가액으로 단순화하는 편이 이해하기 쉽고 유지보수도 쉬움.

**영향 범위:**
- `StockSettingsModal.jsx`: CASH 섹션·잔고부족 경고 제거, 그룹 데이터에 `contributions` 필드 더 이상 생성 안 함
- `StockStatsOverlay.jsx`: 납입금 바차트·CASH 요약 제거, 수익률 계산식을 "매수총액 대비 평가액"으로 변경
- `routers/portfolio.py`: 히스토리 백필 시 현금 잔고를 그룹 총액에 더하던 로직 제거 → 순수 보유 종목 평가액만 반영

**하위호환:** 기존에 저장된 `contributions` 데이터는 DB에 남아있지만 어떤 화면에서도 읽지 않음
(죽은 필드로 방치, 별도 마이그레이션 없음).

**대안:**
- 기존 방식 유지 (납입금 기준 원금 계산): 실제 입금 시점을 정확히 반영하지만 사용자가 매번 입금 기록을 남겨야 해서 누락되기 쉬움
- 선택한 방식: 매수총액 기준 원금 계산 — 이미 입력하는 매수 기록만으로 자동 계산되어 추가 입력 부담 없음

**파일:** `frontend/src/pages/index/StockSettingsModal.jsx`, `frontend/src/pages/index/StockStatsOverlay.jsx`, `routers/portfolio.py`
