...
#### 문제
NVDY처럼 배당을 자주(주간) 지급하고 자동 재투자하는 종목에서, 대시보드 보유수량이
실제 계좌보다 계속 모자람 (예: 실제 104.939주 vs 대시보드 96.883주, 약 8주 차이).

#### 원인
AI 캡처 프롬프트가 "Buy, Sell 행만 추출, Dividend/**Reinvestment**/Sweep/Interest/Fee/
Transfer는 전부 무시"로 지시하고 있었음. 근데 **Reinvestment(배당 재투자)는 실제로
주식을 매수하는 거래**라 Sweep이나 Fee 같은 진짜 무관한 행과 성격이 다름 — 배당금
현금 지급(Dividend)과 그 돈으로 산 주식(Reinvestment)이 항상 쌍으로 찍히는데,
지금까지 이 Reinvestment 매수분이 통째로 씹히고 있었음. 고배당 ETF는 재투자가
매주 0.7~0.9주씩 잦게 일어나므로 누적되면 수량 차이가 크게 벌어짐.

#### 수정
- `routers/portfolio.py`의 AI 캡처 프롬프트 수정: Reinvestment 행을 `type: "buy"`로
  추출하도록 변경 (Dividend 현금 지급 행 자체는 계속 제외)
- 프롬프트에 "같은 날짜에 Dividend/Reinvestment가 쌍으로 나오면 Dividend만 제외,
  Reinvestment는 buy로 추출" 명시적 지시 추가

#### 확인
- `pytest` 50개 통과 (프롬프트 텍스트 변경이라 로직 테스트 대상 아님)
- 실제 캡처 결과는 사용자가 다음 AI 캡처 업로드 시 확인 필요 (Anthropic API 호출 비용
  발생하는 부분이라 이 세션에서 실제 호출 테스트는 하지 않음)

#### 참고
- NVDY 기존 누락분(~8주)은 이 수정으로 자동 소급 반영되지 않음. 과거 명세서를
  AI 캡처로 다시 업로드하면 이번엔 Reinvestment 행까지 인식되어 누락분이 채워짐
  (같은 날짜+수량 조합은 중복 방지 로직이 걸러줌)

---

## 2026-09-15

### 버그 수정 — 같은 티커가 같은 그룹 안에서 별개 종목으로 중복 등록되는 문제

#### 문제
QQQM처럼 이미 그룹에 있는 종목을 나중에 다시 매수 입력(수동 추가 또는 AI 캡처)하면,
기존 종목에 합쳐지지 않고 완전히 별개의 종목으로 새로 생성됨 (예: "12주"와 "2주"로
쪼개져 표시).

#### 원인
종목 티커를 비교할 때 `.toUpperCase()`(대소문자 정규화)만 하고 **앞뒤 공백은
제거(trim)하지 않아서** 발생. 티커 입력 필드(수동 추가, AI 캡처 결과 수정)에
실수로 공백이 하나라도 섞여 저장되면(예: `"QQQM "`), 화면엔 똑같이 "QQQM"으로
보여도 문자열 비교(`"QQQM" === "QQQM "`)는 실패해 "새 종목"으로 처리됨.

백엔드 AI 캡처 응답(`/api/portfolio/parse-transactions`)은 새로 들어오는 티커는
`.strip()`으로 공백을 제거했지만, **그룹에 이미 있는 종목 목록(`existing_tickers`)을
만들 때는 공백 제거를 안 해서** 같은 종류의 비대칭 버그가 한 군데 더 있었음.

#### 수정
- `StockSettingsModal.jsx`: `addStock`, `handleCaptureSuccess`, AI 캡처 결과 검토
  화면의 티커 수정 입력, 검색 자동완성 매칭 — 티커 비교·저장 시 전부 `.trim()` 추가
  (기존에 이미 공백이 섞여 저장된 데이터와도 비교 시점에 trim해서 매칭되도록 처리)
- `routers/portfolio.py`: `/parse-transactions`의 `existing_tickers` 생성 시에도
  `.strip()` 추가

#### 확인
- `npm run build`, `pytest` 50개 통과
- 기존에 잘못 쪼개진 QQQM 데이터 자체는 이 수정으로 자동 정리되지 않음 —
  사용자가 "내 주식 설정"에서 두 항목을 수동으로 병합해야 함
...
#### 문제
AI 캡처로 증권사 명세서를 업로드하면, 명세서가 매도 수량을 관례상 음수로 표기하는 경우
(`-5.0000`) 그 부호를 그대로 가져와 `-5주`로 저장됨. 매수/매도 구분은 이미 `type` 필드로
하고 있으므로 수량은 항상 양수여야 하는데, 음수가 섞이면 보유수량 계산(`매수합 - 매도합`)이
틀어질 수 있음.

#### 수정
- `routers/portfolio.py` (`/api/portfolio/parse-transactions`)
  - AI 파싱 결과의 `qty`/`price`를 항상 절대값으로 저장하도록 변경
  - AI 프롬프트에도 "매도 행이 음수/괄호로 표기돼도 절대값으로 변환" 지시 추가
- `StockSettingsModal.jsx`
  - 수동 매입/매도/수정 폼에서 수량 0 이하, 단가 음수 입력 시 저장 거부
  - AI 캡처 결과 검토 화면에서 수량/단가 수정 시에도 항상 절대값으로 정규화

#### 확인
- 브라우저에서 수량 `-5` 입력 후 매입 시도 → "수량은 0보다 큰 값을 입력해주세요" 거부,
  내역 미추가 확인
- `npm run build`, `pytest` 50개 통과
...
### 주식 관리 — 매수/매도 수동 입력 시 중복 방지

#### 배경
증권사 거래내역을 보면서 수동으로 매수/매도를 입력할 때, 이미 입력한 내역과 겹치는지
알기 어려워 중복 입력 위험이 있었음.

#### 변경 내용
- `StockSettingsModal.jsx` (`StockDetailPanel`)
  - 매입일/매도일 입력 중인 날짜와 같은 날짜의 기존 거래 내역을 거래 내역 목록에서
    노란 테두리 + "⚠ 같은 날짜" 배지로 하이라이트 (입력 전에 미리 확인 가능)
  - 매입/매도 제출 시 같은 날짜 + 같은 수량의 기존 내역이 있으면 즉시 저장하지 않고
    확인 모달로 경고 ("이미 같은 날짜에 N주 매입 내역이 있습니다. 중복 입력일 수
    있어요. 그래도 추가하시겠습니까?") — 확인을 눌러야 실제로 추가됨

#### 확인
- 로컬에서 동일 날짜/수량 재입력 시나리오 테스트: 입력 중 하이라이트 표시, 제출 시
  확인 모달 정상 노출, 취소 시 미추가 확인 (브라우저 직접 클릭 테스트)
- `npm run build` 성공
...
### 코드 품질 — 에러를 조용히 삼키는 except 블록 전수 점검 및 로깅 추가

#### 배경
오늘 발견한 두 버그(주식 그룹 조회 401→빈 배열, 탈퇴 job의 datetime 이름 충돌)가 전부
"예외를 잡고 로그 없이 기본값만 반환"하는 패턴이었음. 백엔드 전체에서 같은 패턴을 훑어
재발 방지 차원에서 로깅 추가.

#### 점검 범위
`routers/*.py`, `main.py`의 `except Exception:` / `except:` 블록 총 42개 확인.
그중 로그도 안 남기고 raise도 안 하는 "완전 침묵" 블록만 선별해 위험도별로 처리:

- **수정함 (사용자 데이터 손상을 숨길 수 있는 것)**: `portfolio.py`의 `portfolio_groups.data`
  JSON 파싱 실패 지점 7곳(그룹 조회, 백필, AI캡처, 이름변경 감지·동기화, 최초매입일 계산),
  `timezone.py`의 저장된 timezone 설정 파싱 실패
- **수정함 (외부 API 연동 실패를 진단 불가능하게 만드는 것)**: Google/Facebook/Kakao/Naver
  OAuth 콜백의 토큰교환·유저정보조회 실패 6곳, Google 이메일 조회 실패
- **수정함 (서버 시작 시 스키마 마이그레이션이 조용히 스킵되는 것)**: `main.py`의
  `_migrate_*` 함수들 — 컬럼/테이블 조회 실패 시 로그 없이 리턴하던 11곳
- **그대로 둠 (정상적인 기본값 폴백이라 로그가 오히려 노이즈)**: 날짜 파싱 실패 시
  `None`/`[]` 반환하는 유틸리티 함수들(`_shared.py`, `todos.py`), 개별 종목 시세 조회
  실패(원래 배치 중 일부 실패를 전제로 설계됨), CHANGELOG 파일 없을 때 빈 배열 반환,
  디버그 미들웨어의 요청 바디 파싱 폴백

#### 확인
- 수정한 모든 파일 `ast.parse` 문법 검증 통과
- `pytest` 50개 전부 통과
- 로컬 서버 재시작 후 정상 부팅, 새 에러 없음 확인
...
#### 변경 내용
- `StockSettingsModal.jsx`: 그룹당 종목 10개 제한 완전 삭제
  - `addStock()`의 개수 체크 제거
  - "최대 10개 종목 도달" 안내 문구 제거 (항상 `AddStockRow` 노출)
  - 그룹 헤더의 `종목수/10` 배지 → `종목수개`로 변경 (상한 없음을 반영)
  - 그룹 자체의 최대 10개 제한은 유지 (요청 범위 아님)
- `en.json`/`ko.json`: `admin.stockHint`에서 "그룹당 종목 최대 10개" 문구 삭제
- `stockSummary()`에 `lastTxDate` 추가 — 매수/매도 기록 중 가장 최근 날짜 계산
- 종목 요약 줄에 `· 마지막 거래일 YYYY-MM-DD` 표시 (기록 없으면 미표시)

#### 확인
- 로컬 회원가입 → 종목 12개 추가 테스트: 제한 문구 없이 정상 추가됨 확인
- 매입 기록 추가 후 "마지막 거래일 2026-04-04" 정상 표시 확인 (브라우저 직접 클릭 테스트)
- `npm run build` 성공
