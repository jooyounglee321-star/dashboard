...
### 백필 현금 잔고 제거 — 실시간 vs 히스토리 총액 불일치 수정

- `backfill_portfolio_snapshots()`에서 그룹별 현금 잔고(매도수익 − 재매수비용) 계산 블록 전체 제거
- 원인: 백필은 미투자 현금을 그룹 총액에 가산했으나 실시간 StockCard는 현금을 제외 → 60% 괴리 발생
- 수정 후 양쪽 모두 "보유 주식 평가액만" 기준으로 통일
- `POST /api/portfolio/backfill-full` 로 기존 히스토리 전체 재계산 필요
